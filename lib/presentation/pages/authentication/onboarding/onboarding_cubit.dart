...
class OnboardingCubit extends Cubit<OnboardingState> {
  ...
  List<Widget> get onBoardingSteps => [
        /// 1st part
        const PersonalDetailsIntro(),
        const PersonalDetailsNameSection(),
        const PersonalDetailsDobSection(),
        const PersonalDetailsEthnicitySection(),
        const PersonalDetailsGenderSection(),
        const PersonalDetailsVeteranSection(),
        PersonalDetailsLocationSection(),
        const PersonalDetailsPhoneSection(),

        /// 2nd part
        const IncarcerationDetailsIntro(),
        IncarcerationDetailsIdNumber(),
        const IncarcerationDetailsIncarceratedTime(),
        const IncarcerationDetailsIncarcerationHistory(),
        const IncarcerationDetailsOffencesSection(),
        const IncarcerationDetailsIncarcerationLength(),
        const IncarcerationDetailsLatestIncarceration(),
        const IncarcerationDetailsFirstIncarceratedDate(),
        const IncarcerationDetailsLatestReleaseDate(),
        const IncarcerationDetailsRecentServe(),
        const IncarcerationDetailsProgramsSection(),

        /// 3rd part
        const CurrentNeedsIntro(),
        const CurrentNeedsTopPriorities(),
        const CurrentNeedsServiceProviderType(),
        const CurrentNeedsHousingStatus(),
        const CurrentNeedsEducationLevel(),
        const CurrentNeedsTradeCertifications(),
        const CurrentNeedsSkillsToImprove(),
        const CurrentNeedsEmploymentStatus(),
        const CurrentNeedsCurrentCareer(),
        const CurrentNeedsSalaryLevel(),
        const CurrentNeedsCareerChange(),
        const CurrentNeedsFutureCareer(),
        const CurrentNeedsExpectedSalary(),
        const CurrentNeedsOtherResource(),

        /// 4th part
        const ServiceProvidersIntro(),
        ServiceProvidersAccessedSoFar(),
        const ServiceProvidersAccessDates(),
      ];
